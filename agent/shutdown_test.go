...
// readFrameType reads one frame and returns its type. frame.Read cannot be used
// here: its payload switch rejects agent-side frames outright, because it only
// decodes what an agent receives. The status code on an AGENT-DISCONNECT is
// pinned by the worker package's drain test; what matters here is the ordering.
...
// serveOnAnEphemeralPort starts the agent and hands back its address and a
...
// dialAndHandshake connects and completes the HELLO, returning the connection
// and a reader positioned after the AGENT-HELLO.
...
// The whole point: a handler mid-flight at shutdown finishes, its ACK goes out,
// and only then does the peer get its goodbye.
func TestAgent_ShutdownDrainsAnInFlightHandler(t *testing.T) {
	...
	// Serve stops as soon as the listener closes; it does not wait for the drain.
	select {
	case err := <-served:
...
// A handler that respects its context is cancelled when the grace period goes,
// and Shutdown reports that the drain did not finish in time.
func TestAgent_ShutdownCancelsHandlersWhenTheGracePeriodExpires(t *testing.T) {
	entered := make(chan struct{})
...
// A handler that ignores its context entirely must not hold Shutdown past the
// grace period. Go cannot stop such a goroutine, so the bound has to come from
// ctx rather than from the drain finishing.
func TestAgent_ShutdownDoesNotWaitForAHandlerThatIgnoresItsContext(t *testing.T) {
	entered := make(chan struct{})
	release := make(chan struct{})

	// Released only at the end of the test, so the handler is still running
	// when Shutdown gives up on it.
...
// Nothing connected is the common case in a test binary and at process start.
func TestAgent_ShutdownWithNoConnections(t *testing.T) {
	a := New(noopHandler, logger.NewNop())
...
// A second Shutdown must not panic closing an already-closed channel.
...
// Serving on an agent that has already drained refuses, rather than accepting
// work that nothing will finish.
func TestAgent_ServeAfterShutdown(t *testing.T) {
	a := New(noopHandler, logger.NewNop())
